...
export const Header: React.FC<HeaderProps> = ({ className = "" }) => {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        ...
        !target.closest(".currency-menu-container")
      ) {
        setIsUserMenuOpen(false);
  ...
  return (
    <header
      className={`bg-white shadow-sm border-b border-gray-100 sticky top-0 z-40 backdrop-blur-sm bg-white/95 ${className}`}
    >
        ...
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex items-center space-x-1">
            ...
            <div className="relative user-menu-container">
              {isLoading ? (
                // Loading state - Skeleton pendant le chargement initial
                ...
                >
                  <div className="w-8 h-8 bg-gradient-to-br from-purple-500 to-blue-600 rounded-full flex items-center justify-center shadow-sm">
                    <span className="text-white text-sm font-semibold">
                      {data.user.profile?.data?.firstName &&
                      data.user.profile?.data?.lastName
                        ? `${data.user.profile.data.firstName.charAt(0)}${data.user.profile.data.lastName.charAt(0)}`
                        : data.user.profile?.data?.firstName?.charAt(0) ||
                          data.user.profile?.data?.email?.charAt(0) ||
                          "U"}
                    </span>
                  </div>
                      ...
                      <div className="flex items-center space-x-3 px-4 pb-4 border-b border-gray-100">
                        <div className="flex-1">
                          ...
                          {/* <div className="text-base font-normal text-gray-900 mb-1">
                            {data.user?.profile?.data?.fullName}
                          </div> */}
