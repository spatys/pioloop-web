...
const PropertyDateRangePicker: React.FC<PropertyDateRangePickerProps> = ({
  ...
  const isUnavailable = (date: Date) => {
    ...
    return unavailableDates.some(unavailableDate => 
      format(unavailableDate, "yyyy-MM-dd") === dateString
    );
  };

  // Vérifier si une date est bloquée (indisponible ou après une date indisponible)
  const isBlocked = (date: Date) => {
    if (isUnavailable(date)) return true;
    
    // Si une date de départ est sélectionnée, bloquer toutes les dates après la prochaine date indisponible
    if (selectedDates.from) {
      const nextUnavailableDate = unavailableDates
        .filter(unavailableDate => isAfter(unavailableDate, selectedDates.from!))
        .sort((a, b) => a.getTime() - b.getTime())[0];
      
      if (nextUnavailableDate && isAfter(date, nextUnavailableDate)) {
        return true;
      }
    }
  ...
  const handleDateClick = (date: Date) => {
    // Empêcher la sélection des dates passées ou indisponibles
    if (isBefore(startOfDay(date), startOfDay(new Date())) || isUnavailable(date)) {
      return;
    }
    ...
    if (!selectedDates.from || (selectedDates.from && selectedDates.to)) {
      ...
      // Deuxième sélection - vérifier qu'il n'y a pas de dates indisponibles dans la plage
      const startDate = date < selectedDates.from ? date : selectedDates.from;
      const endDate = date < selectedDates.from ? selectedDates.from : date;
      
      // Vérifier s'il y a des dates indisponibles dans la plage
      const hasUnavailableInRange = unavailableDates.some(unavailableDate => 
        isAfter(unavailableDate, startDate) && isBefore(unavailableDate, endDate)
      );
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  onMouseEnter={() => !isPast && !isUnavailableDate && setHoveredDate(day)}
                  onMouseLeave={() => setHoveredDate(null)}
                  disabled={isPast || isUnavailableDate || isBlockedDate}
                  className={`
                  ...
                  ${
                    isPast || isUnavailableDate || isBlockedDate
                      ? "text-gray-300 cursor-not-allowed bg-gray-100"
                      : "hover:bg-purple-50 hover:text-purple-700 cursor-pointer text-gray-900"
                  }
                  ${
                    isCurrentDay && !isSelected && !isInSelectedRange && !isPast && !isUnavailableDate && !isBlockedDate
                  ...
                  ${
                    isHovered && !isSelected && !isInSelectedRange && !isPast && !isUnavailableDate && !isBlockedDate
