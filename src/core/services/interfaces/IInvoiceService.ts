...
export interface IInvoiceService {
  ...
  generateRecurringInvoices(propertyId: string, startDate: Date, endDate: Date): Promise<Invoice[]>;
  
  // Invoice validation
  validateInvoice(data: CreateInvoiceForm): Promise<{ isValid: boolean; errors: string[] }>;
  ...
  calculateInvoiceTotal(items: any[]): Promise<{ subtotal: number; taxAmount: number; totalAmount: number }>;
} 