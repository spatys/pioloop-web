...
export class ContractService implements IContractService {
  ...
  async getContractsByTenant(tenantId: string): Promise<Contract[]> {
    const response =
      await this.contractRepository.getContractsByTenant(tenantId);
    if (!response.success) {
      throw new Error(
        response.message || "Failed to fetch contracts by tenant",
      );
    }
    return response.data;
  }
  ...
  async getContractsByProperty(propertyId: string): Promise<Contract[]> {
    const response =
      await this.contractRepository.getContractsByProperty(propertyId);
    if (!response.success) {
      throw new Error(
        response.message || "Failed to fetch contracts by property",
      );
    }
    return response.data;
  }
  ...
  async updateContractStatus(
    ...
    const response = await this.contractRepository.updateContractStatus(
      id,
      status,
      reason,
    );
  ...
  async signContract(
    ...
    const response = await this.contractRepository.signContract(
      id,
      signatureType,
    );
    if (!response.success) {
  ...
  async generateContractFromReservation(
