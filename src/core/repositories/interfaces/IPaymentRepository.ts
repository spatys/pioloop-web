...
export interface IPaymentRepository {
  ...
  getPaymentsByReservation(reservationId: string): Promise<ApiResponse<Payment[]>>;
  ...
  updatePaymentStatus(id: string, status: string): Promise<ApiResponse<Payment>>;
  
  // Stripe integration
  createStripePaymentIntent(amount: number, currency: string): Promise<ApiResponse<{ clientSecret: string }>>;
  confirmStripePayment(paymentIntentId: string): Promise<ApiResponse<Payment>>;
  
  // Payment validation
  validatePayment(data: CreatePaymentForm): Promise<ApiResponse<{ isValid: boolean; errors: string[] }>>;
